...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dscoresx, dscoresw, dscoresb = affine_backward(dsoftmax, scores_cache)
        dhiddenx, dhiddenw, dhiddenb = affine_relu_backward(dscoresx, hidden_cache)

        grads['W2'] = dscoresw + self.reg * W2
        grads['W1'] = dhiddenw + self.reg * W1
        grads['b2'] = dscoresb
        grads['b1'] = dhiddenb
        ############################################################################
        #                             END OF YOUR CODE                             #
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.use_batchnorm:
                output, cache = affine_batchnorm_relu_forward(output, self.params[key_w], self.params[key_b], self.params[key_gamma], self.params[key_beta], self.bn_params[i])  
            elif self.use_dropout:
                output, cache = affine_relu_dropout_forward(output, self.params[key_w], self.params[key_b], self.dropout_param)
            else:           
                output, cache = affine_relu_forward(output, self.params[key_w], self.params[key_b])
            caches.append(cache)
            
        key_w = 'W' + str(self.num_layers - 1)
        key_b = 'b' + str(self.num_layers - 1)
        scores, cache = affine_forward(output, self.params[key_w], self.params[key_b])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        loss += 0.5 * self.reg * np.sum(self.params[key_w] * self.params[key_b])
        
        dx, dw, db = affine_backward(dl, cache)        
        grads[key_w] = dw + self.reg * self.params[key_w]
        grads[key_b] = db
        ...
        for i in range(self.num_layers - 2, -1, -1):
            ...
            if self.use_batchnorm:
                ...
                dx, dw, db = affine_relu_dropout_backward(dx, caches[i])
            else:
