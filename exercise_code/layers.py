...
def batchnorm_backward(dout, cache):
    ...
    dmean = np.sum(- 1 / np.sqrt(var + eps) * dnormalized_x, axis=0)
    dvar = np.sum((-x_minus_mean) / (2 * (var + eps) ** (3/2)) * dnormalized_x, axis=0)
    dx = 1 / np.sqrt(var + eps) * dnormalized_x + dvar * 2.0 / N * (x_minus_mean) + 1.0 / N * dmean
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
